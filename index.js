...
client.on('ready',  async() => {
    ...
    const statusBruh = ['Minecraft, PLAYING', 'Fortnite, PLAYING', 'Spotify, LISTENING', 'Your Commands, WATCHING']

    setInterval(() => {
        const random = statusBruh[Math.floor(Math.random() * statusBruh.length)].split(', ')
...
if(number >= 4) {
    ...
    var array = ['bad' , 'words', 'go' , 'here', 'if you are seeing this, pls sub to my channel :)'];
        ...
        if(array.some(w =>  ` ${msg.content.toLowerCase()} `.includes(` ${w} `))){
            ...
            var animeBonk = msg.guild.emojis.cache.find(emoji => emoji.name === 'bonk') //PUT YOUR EMOJI NAME HERE
